...
TEST_CASE( "sjbes" ){
  ...
  GIVEN( "compute normal values" ){
    ...
    WHEN( "0 < n <= 10" ){
      double x = 6e-4;
      THEN( "output is correct" ){
        equal( sjbes(1,x), 2.0E-4 );
        equal( sjbes(5,x), 7.480521E-21 );
        equal( sjbes(10,x), 4.39776E-43 );
      } // THEN
    } // WHEN
    WHEN( "n > 10" ){
      double x = 6e-4;
      THEN( "output is correct" ){
        equal( sjbes(11,x), 0.0 );
      } // THEN
    } // WHEN
  } // GIVEN
  ...
  GIVEN( "not compute normal values" ){
    ...
    WHEN( "x < 0.2 and n != 0" ){
      double x = 7e-4;
      THEN( "output is correct" ){
        equal( sjbes(1,x), 2.333333E-4 );
      } // THEN
    } // WHEN
    WHEN( "x >= 0.2 and n = 0" ){
      double x = 0.2;
      THEN( "output is correct" ){
        equal( sjbes(0,x), 0.99334665 );
      } // THEN
    } // WHEN
    WHEN( "x >= 0.2 and n != 0" ){
      double x = 0.2;
