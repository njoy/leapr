...
void equalVec1( const std::vector<double>& a, const std::vector<double>& b ){
...
TEST_CASE( "normalize" ){
  ...
  GIVEN( "some vector p, spacing delta, normalizing value tbeta" ){
    ...
    delta = 0.2; tbeta = 2.0;
    normalize( p, delta, tbeta );
    correct = { 5.2978935E-2, 0.10595787, 0.15893680, 0.21191574, 0.26489467, 
      0.31787361, 0.37085254, 0.42383148, 0.47681042, 0.52978935, 0.58276829, 
      0.63574722 };
  ...
  GIVEN( "two vectors where one is the first scaled by some constant" ){
    ...
    correct = { 3.095826174E-2, 6.191652348E-2, 9.287478752E-2, 0.1238330469, 
      0.1547913063, 0.1857495750};

    THEN( "outputs should be same, and both within 1e-6 to true value" ){
      equalVec1( p1, correct );
      equalVec1( p2, correct );
    } // THEN
  } // GIVEN
} // TEST CASE
