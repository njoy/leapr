...
//                                            through my happy temp loop
  ...
  auto ff1_2 = ranges::view::zip_with( [](auto ff2,auto be){
                 return ff2*exp(-be); }, ff2_2, be2 );
  ...
  for ( size_t a = 0; a < alpha.size(); ++a ){
    if ( ( a % naint == 0 ) or ( a == alpha.size() - 1) ){
      ...
      for ( size_t b = 0; b < beta.size(); ++b ){
        ...
        std::cout << ff1l << "   " << ff1 << "   " << ff2l << "   " << ff2 << std::endl;
      ...
      //std::cout << "sum0:  " << sum0 << "     alpha:  " << a << std::endl;
