  ...
  GIVEN( "some vector, its spacing, and desired x value" ){
    ...
    WHEN( "desired point is not one of the explicitly defined values" ){
      THEN( "value is interpolated correctly" ){
        ...
        REQUIRE(interpolate(p,0.5,1.2,betaGrid) == Approx(3.8).epsilon(1e-7) );
        ...
        REQUIRE( interpolate(p,0.35,4.0,betaGrid) == Approx(2.48571433).epsilon(1e-7) );

        REQUIRE( interpolate(p,0.35,6.0,betaGrid) == Approx(4.51428572).epsilon(1e-7) );
    ...
    WHEN( "desired point is an explicitly defined value" ){
      THEN( "value is exactly returned" ){
        for (size_t i = 0; i < betaGrid.size(); ++i){ betaGrid[i] = i*0.5; }
        REQUIRE( interpolate(p,0.5,0.0,betaGrid) == 1.0 );
        REQUIRE( interpolate(p,0.5,1.5,betaGrid) == 5.0 );
      } // THEN
    } // WHEN
    ...
    WHEN( "desired point is out of range (at highest point or above)" ){
      THEN( "a value of 0.0 is returned" ){
        for (size_t i = 0; i < betaGrid.size(); ++i){ betaGrid[i] = i*0.5; }
        REQUIRE( interpolate(p,0.5,2.50,betaGrid) == 0.0 );
        REQUIRE( interpolate(p,0.5,2.51,betaGrid) == 0.0 );
      } // THEN
    } // WHEN
  } // GIVEN
}
