...
auto betaLoop( const Range& betan, 
  const Range& rdbex, const Range& bex,
  const Range& sex, const Float& alphaVal, const Float& wt, 
  const Float& tbart, const Float& x, const Float& y, 
  const Float& evenSumConst, const Float& oddSumConst, int itemp, int nbx, 
  int a, int ncold, bool free, 
  Range& sym_sab_1, 
  Range& sym_sab_2 ){
  ...
  unsigned int jjmax = 2 * betan.size() - 1; 
  int k;
  Float pj, be, snlg, snlk, sn, total;
