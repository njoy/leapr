...
TEST_CASE( "skold" ){
  double cfrac = 0.3, tev = 0.0172348, awr = 8.9, dka = 2.3;
  ...
  std::vector<double> sab = ranges::view::iota(1,26);

  GIVEN( "medium sized skappa values" ){
    WHEN( "dka is a bit large" ){
      THEN( "the scattering law is correctly changed" ){
        skold( cfrac, tev, alpha, beta, skappa, awr, dka, sab );
        REQUIRE( ranges::equal(sab, rightSymSab, equal) );
      } // THEN
    } // WHEN
