...
auto bfact( const Float& x, const Float& alpha_lambda_i, const Float& beta_i, 
  ...
  if ( y <= 1.0 ){
    ...
    Range I1Consts = {0.0360768, 0.2659732,  1.2067492,  3.0899424,  3.5156229, 1.0};
    Range I2Consts = {0.0030153, 0.02658733, 0.15084934, 0.51498869, 0.87890594, 0.5};
    I0 = 0.0045813;
    I1 = 0.00032411;
    auto ySquared = y*y;
    for ( auto entry : I1Consts ){ I0 = I0 * ySquared + entry; }
    for ( auto entry : I2Consts ){ I1 = I1 * ySquared + entry; }
  ...
  if ( y > 1.0 ) {
    ...
    // Compute I0 and I1 via series expansion (manual pg. 714 top)
    Range I1Consts { -0.01647633, 0.02635537, -0.02057706, 0.00916281, -0.00157565, 
      0.00225319, 0.01328592, 0.39894228 };
    Range I2Consts { 0.01787654, -0.02895312, 0.02282967, -0.01031555, 0.00163801, 
      -0.00362018, -0.03988024, 0.39894228 };
    I0 = 0.00392377;
    I1 = -0.00420059;
    auto yInv = 1.0/y;
    for ( auto entry : I1Consts){ I0 = I0 * yInv + entry; }
    for ( auto entry : I2Consts){ I1 = I1 * yInv + entry; }
  ...
  auto cutoff = [](auto x){ return x < 1.0e-30 ? 0.0 : x; };

  In = ranges::view::concat( 
         ranges::view::single(cutoff(I1)),
         In | ranges::view::transform([frac=I1/In[0]](auto x){return x*frac;})
            | ranges::view::transform(cutoff)
            | ranges::view::drop(1) );
