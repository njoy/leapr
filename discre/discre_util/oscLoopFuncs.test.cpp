...
void equal( double a, double b ){
  if (b == 0.0){ 
    REQUIRE( std::abs(b-a) < 2e-6 );
    return;
  }
  REQUIRE ( std::abs( (a-b)/(b) ) < 2e-6 );
}
...
TEST_CASE( "oscillator loop" ){
  GIVEN( "inputs" ){
    ...
    std::vector<double> wts (maxdd, 0.0), bes(maxdd, 0.0), energyNorm(50, 0.0), 
      dbw(50, 0.0), wtn(maxdd, 0.0), ben(maxdd, 0.0), ar(50, 0.0), dist(50,0.0);
    ...
    oscillatorLoop( alpha, dbw, ar, scaling, wts, wtn, bes, ben, energyNorm, 
    ...
    THEN( "ouput is correct" ){
      ...
      for ( auto i = 0; i < wts.size(); ++i ){ 
        if ( i < wtsCorrect.size() ){ equal( wts[i], wtsCorrect[i] ); } 
        else { equal( wts[i], 0.0 ); }
      }

      for ( auto i = 0; i < bes.size(); ++i ){ 
        if ( i < besCorrect.size() ){ equal( bes[i], besCorrect[i] ); } 
        else { equal( bes[i], 0.0 ); }
      }

      equal( wt, 3.0 );
      equal( tbart, 407.4545311 );
